...
export default function CustomDrawerContent({
  ...
  // ❌ HAPUS SWIPE SETTINGS YANG TIDAK DIGUNAKAN
  // Kita hanya butuh load auth data, swipe settings sudah di handle di DrawerNavigator
    ...
    <View style={[styles.container, { maxHeight: height * 0.9 }]}>
      <View style={styles.header}>
        <View style={styles.userInfo}>
          {isLoggedIn && user ? (
            <>
              {/* ✅ SETELAH LOGIN: TAMPILKAN NAMA USER */}
              <Text style={styles.userName}>{user.name}</Text>
              <Text style={styles.userEmail}>{user.email}</Text>
            </>
          ) : (
            <>
              {/* ✅ SEBELUM LOGIN: TAMPILKAN GUEST */}
        ...
        <View style={styles.avatarContainer}>
          {isLoggedIn && user ? (
            // ✅ SETELAH LOGIN: AVATAR DENGAN HURUF AWAL KAPITAL
...
const styles = StyleSheet.create({
  ...
  // 🆕 STYLE UNTUK AVATAR DENGAN HURUF
  avatarWithLetter: {
    width: 50,
    height: 50,
    borderRadius: 25,
