...
const ProfileContent: React.FC<{
  ...
  const navigation = useNavigation<ProfileScreenNavigationProp>(); // ✅ TAMBAHKAN INI
  const userStats = {
    orders: 15,
    favorites: 8,
    discounts: 12,
    ...
    if (params?.fromDeepLink && params?.deepLinkUserId) {
      ...
      if (validationResult.success && validationResult.userId) {
        ...
        if (userData) {
          ...
          // Di real app, kita akan update user state di sini
          // Untuk sekarang, kita hanya set parameters untuk UI
          setDeepLinkParams({
            fromDeepLink: true,
