...
// 🆕 SEMUA KOMPONEN DIPINDAH KE LUAR
...
// 🆕 HOOK UNTUK SWIPE SETTINGS REAL-TIME - DIPINDAH KE LUAR
const useSwipeSettingsRealTime = () => {
  ...
  useEffect(() => {
    ...
    // 🆕 LISTENER UNTUK SETTINGS CHANGE - Check every 500ms
    const intervalId = setInterval(loadSwipeSetting, 500);
...
const getCheckoutOptions = ({ navigation }: any) => ({
  ...
  headerShown: true,
  headerLeft: () => (
    <ModalHeaderLeft navigation={navigation} />
  ),
  swipeEnabled: false,
});

// 🆕 SCREEN OPTIONS CREATOR - DIPINDAH KE LUAR
const createScreenOptions = (
  ...
  return ({ navigation, route }: any) => ({
    ...
    // ✅ HEADER LEFT TANPA SWIPE STATUS (CLEAN)
