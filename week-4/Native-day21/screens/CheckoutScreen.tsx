...
export default function CheckoutScreen() {
  ...
  // ✅ STATE UNTUK SHIPPING CALCULATION
  ...
  // ✅ TAHAP 2: Calculate shipping dengan optimized location
  const calculateShipping = async () => {
    try {
      setIsCalculatingShipping(true);
      setShippingCost(null);

      console.log('🚚 Calculating shipping with optimized location...');
      ...
      if (location) {
        ...
        // Simulate shipping cost calculation based on distance
        const baseCost = 15000;
        const distanceCost = Math.round(location.accuracy || 0) / 1000 * 2000;
        const calculatedCost = baseCost + Math.min(distanceCost, 50000);
        ...
        Alert.alert(
          ...
          `Ongkos kirim: Rp ${calculatedCost.toLocaleString('id-ID')}\nLokasi terdeteksi dengan akurasi ${Math.round(location.accuracy || 0)} meter`,
          [{ text: 'OK', style: 'default' }]
        );
      } else {
        Alert.alert(
          'Gagal Menghitung Ongkir',
          'Tidak dapat menentukan lokasi Anda untuk menghitung ongkos kirim.',
          [{ text: 'OK', style: 'default' }]
        );
      }
    } catch (error: any) {
      ...
      if (error.message.includes('GPS timeout')) {
        // Error sudah dihandle di locationService
      } else {
        Alert.alert(
          'Error',
  ...
  const handleBiometricConfirmation = async () => {
    ...
    try {
      const totalAmount = getTotalPrice() + (shippingCost || 0);
  ...
  const handleManualConfirmation = () => {
    Alert.alert(
      'Konfirmasi Manual',
      `Anda akan melakukan checkout sebesar ${formatRupiah(getTotalPrice() + (shippingCost || 0))}. Lanjutkan?`,
      [
        { text: 'Batal', style: 'cancel' },
        {
          text: 'Ya, Lanjutkan',
  ...
  const handleCheckoutFinal = async () => {
    ...
    try {
      ...
      if (paymentResult.success) {
        Alert.alert(
          'Checkout Berhasil! 🎉',
          `Terima kasih telah berbelanja!\n${paymentResult.message}\nTotal: ${formatRupiah(getTotalPrice() + (shippingCost || 0))}`,
          [
            {
              text: 'OK',
              onPress: () => {
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <View style={styles.summaryCard}>
            ...
            {/* ✅ TAHAP 2: Shipping Cost Section */}
            <View style={styles.shippingSection}>
              <View style={styles.shippingHeader}>
                <Text style={styles.shippingLabel}>Ongkos Kirim</Text>
                <TouchableOpacity 
                  style={styles.calculateShippingButton}
                ...
                >
                  {isCalculatingShipping ? (
                    ...
                    <Text style={styles.calculateShippingText}>Hitung</Text>
                  )}
                </TouchableOpacity>
              </View>
              ...
              {shippingCost !== null ? (
                ...
                <Text style={styles.shippingPlaceholder}>
                  Tekan 'Hitung' untuk estimasi ongkir
                </Text>
              )}
              
              {userLocation && (
                <Text style={styles.locationInfo}>
                  📍 Lokasi: {userLocation.latitude.toFixed(4)}, {userLocation.longitude.toFixed(4)}
                </Text>
              )}
            </View>
            ...
            <View style={styles.summaryRow}>
              ...
              <Text style={styles.totalPrice}>
                {formatRupiah(getTotalPrice() + (shippingCost || 0))}
              </Text>
            </View>
        ...
        <View style={styles.footer}>
          ...
          >
            {isProcessingBiometric ? (
              ...
              <Text style={styles.checkoutButtonText}>🔐 Checkout - {formatRupiah(getTotalPrice() + (shippingCost || 0))}</Text>
            ) : (
              <Text style={styles.checkoutButtonText}>💳 Checkout - {formatRupiah(getTotalPrice() + (shippingCost || 0))}</Text>
            )}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // ✅ TAHAP 2: New styles for shipping section
  shippingSection: {
