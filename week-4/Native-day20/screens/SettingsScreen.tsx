...
export default function SettingsScreen() {
  ...
  const { biometryType, biometricAvailable, checkBiometricAvailability } = useBiometric();
  ...
  const handleSettingChange = async (key: keyof AppSettings, value: boolean) => {
    ...
    if (key === 'swipeDrawer') {
    ...
    } else if (key === 'biometricLogin') {
      if (value && !biometricAvailable) {
        Alert.alert(
          'Biometric Tidak Tersedia',
          `Perangkat Anda tidak mendukung ${biometryType || 'biometric authentication'}.`,
          [{ text: 'Mengerti' }]
        );
        return;
      }
      ...
      if (saved) {
        Alert.alert(
          'Biometric Login',
          value ? 
            `Login ${biometryType === 'FaceID' ? 'Face ID' : 'Touch ID'} diaktifkan!` :
            'Login biometric dimatikan.',
          [{ text: 'OK' }]
        );
      }
    } else {
  ...
  const getBiometricDescription = () => {
    if (!biometricAvailable) {
      return 'Login cepat dengan biometric tidak tersedia di perangkat ini';
    }
    
    if (biometryType === 'FaceID') {
      return 'Login cepat dengan Face ID - Pindai wajah untuk masuk';
    } else if (biometryType === 'TouchID') {
      return 'Login cepat dengan Touch ID - Tempelkan jari untuk masuk';
    } else {
      return 'Login cepat dengan biometric authentication';
    }
  };
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.section}>
        ...
        <SettingItem
          ...
          title={`Login ${biometryType === 'FaceID' ? 'Face ID' : biometryType === 'TouchID' ? 'Touch ID' : 'Biometric'}`}
          description={getBiometricDescription()}
          value={settings.biometricLogin}
          onValueChange={(value) => handleSettingChange('biometricLogin', value)}
          isFunctional={biometricAvailable}
        />
      ...
      <View style={styles.section}>
          ...
          <View style={styles.biometricStatus}>
            ...
            <View style={styles.biometricStatusText}>
              <Text style={styles.biometricStatusTitle}>
                {biometryType === 'FaceID' ? 'Face ID' : biometryType === 'TouchID' ? 'Touch ID' : 'Biometric'} Siap
              </Text>
              <Text style={styles.biometricStatusSubtitle}>
                {biometryType === 'FaceID' ? 
                  'Perangkat mendukung Face ID untuk autentikasi aman' :
                  'Perangkat mendukung Touch ID untuk autentikasi aman'
                }
              </Text>
            </View>
          </View>
        ...
        <SettingItem
          ...
          description={`Login cepat dengan ${biometryType === 'FaceID' ? 'Face ID' : biometryType === 'TouchID' ? 'Touch ID' : 'biometric'}`}
          value={settings.biometricLogin}
          onValueChange={(value) => handleSettingChange('biometricLogin', value)}
        />
      ...
      <TouchableOpacity 
        style={styles.refreshBiometricButton} 
        onPress={checkBiometricAvailability}
      >
        <Text style={styles.refreshBiometricButtonText}>🔄 Refresh Status Biometric</Text>
      </TouchableOpacity>
      ...
      <View style={styles.footerNote}>
        ...
        {biometricAvailable && (
          <Text style={styles.footerText}>
            🔐 <Text style={styles.footerHighlight}>{biometryType === 'FaceID' ? 'Face ID' : 'Touch ID'} tersedia</Text> untuk login cepat dan konfirmasi transaksi.
          </Text>
        )}
      </View>
    </ScrollView>
