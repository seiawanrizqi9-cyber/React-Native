...
interface AuthState {
  ...
  lastAuthUpdate: number; // ✅ BARU: Timestamp untuk force refresh
}
...
export const useAuth = () => {
  const [authState, setAuthState] = useState<AuthState>({
    ...
    securityError: null,
    isLoggingOut: false,
    lastAuthUpdate: Date.now(), // ✅ BARU
  });

  const navigation = useNavigation();

  // ✅ BARU: Function untuk force update global auth state
  const forceAuthUpdate = useCallback(() => {
    setAuthState(prev => ({
      ...prev,
      lastAuthUpdate: Date.now() // Trigger re-render di semua component yang pakai useAuth
    }));
  }, []);
  ...
  const logout = useCallback(async (): Promise<{
    ...
    try {
      ...
      setAuthState({
        ...
        securityError: null,
        isLoggingOut: false,
        lastAuthUpdate: Date.now(), // ✅ BARU: Reset timestamp
      });
  ...
  const login = useCallback(async (username: string, password: string) => {
    try {
      ...
      // ✅ PERBAIKAN PENTING: Update state dengan user data yang baru dan trigger refresh
      setAuthState({
        isLoggedIn: true,
        user: userWithToken,
        token: accessToken,
        ...
        securityError: null,
        isLoggingOut: false,
        lastAuthUpdate: Date.now(), // ✅ BARU: Update timestamp untuk force refresh
      });

      console.log('🎉 Login successful for user:', userData.username);
      
      // ✅ AUTO REFRESH: Trigger global auth state update
      // This will ensure ProfileScreen gets the updated data immediately
      setTimeout(() => {
        loadAuthData(); // Refresh auth data globally
        forceAuthUpdate(); // ✅ BARU: Force update semua component
      }, 100);
  ...
  }, [forceAuthUpdate, loadAuthData]); // ✅ FIXED: Tambah dependencies yang diperlukan
  ...
  const updateUser = useCallback(async (userData: User) => {
    try {
      ...
      setAuthState(prev => ({ 
        ...prev, 
        user: userData,
        lastAuthUpdate: Date.now() // ✅ BARU: Update timestamp
      }));
    } catch (error: any) {
  ...
  return {
    ...
    lastAuthUpdate: authState.lastAuthUpdate, // ✅ BARU: Export timestamp
    login,
    logout,
    updateUser,
    saveUserPreferences,
